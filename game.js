(() => {
  ...
  function spawnPipe(){
    ...
    state.pipes.push({ x, topH, gap, passed: false });
  }

  function spawnHeart(){
    const x = canvas.width + 20;
    const y = rand(90, state.floorY - 190);
    state.hearts.push({ x, y, r: 12, taken: false });
  }
  ...
  function loseLife(){
    ...
    for (const p of state.pipes) p.x += 140;
    for (const h of state.hearts) h.x += 140;
  }
  ...
  function update(){
    ...
    if (state.t % state.heartEvery === 0) spawnHeart();
